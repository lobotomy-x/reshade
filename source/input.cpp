...
reshade::input::input(window_handle window) : _window(window) {}

void reshade::input::register_window_with_raw_input(window_handle window,
                                                    bool no_legacy_keyboard,
                                                    bool no_legacy_mouse) {
  ...
  const auto flags =
      (no_legacy_keyboard ? 0x1u : 0u) | (no_legacy_mouse ? 0x2u : 0u);
  const auto insert =
      s_raw_input_windows.emplace(static_cast<HWND>(window), flags);

  if (!insert.second)
    insert.first->second |= flags;
}
std::shared_ptr<reshade::input>
reshade::input::register_window(window_handle window) {
  assert(window != nullptr);
  ...
  if (process_id != GetCurrentProcessId()) {
    reshade::log::message(
        ...
        "Cannot capture input for window %p created by a different process.",
        window);
    return nullptr;
  }
  ...
  const auto insert =
      s_windows.emplace(static_cast<HWND>(window), std::weak_ptr<input>());

  if (insert.second || insert.first->second.expired()) {
#if RESHADE_VERBOSE_LOG
    reshade::log::message(reshade::log::level::debug,
                          "Starting input capture for window %p.", window);
#endif
...
bool reshade::input::handle_window_message(const void *message_data) {
  ...
  bool is_keyboard_message =
      details.message >= WM_KEYFIRST && details.message <= WM_KEYLAST;
  ...
  if (input_window == s_windows.end()) {
    ...
    EnumChildWindows(
        ...
        [](HWND hwnd, LPARAM lparam) -> BOOL {
          auto &input_window =
              *reinterpret_cast<decltype(s_windows)::iterator *>(lparam);
  ...
  if (input_window == s_windows.end()) {
    // Some applications handle input in a child window to the main render
    // window
  ...
  if (input_window == s_windows.end() &&
      raw_input_window != s_raw_input_windows.end()) {
    ...
    input_window = std::max_element(s_windows.begin(), s_windows.end(),
                                    [](auto lhs, auto rhs) {
                                      return lhs.second.lock()->_frame_count <
                                             rhs.second.lock()->_frame_count;
                                    });
  }
  ...
  // It may happen that the input was destroyed between the removal of expired
  // entries above and here, so need to abort in this case
  if (input == nullptr)
    return false;

  // At this point we have a shared pointer to the input object and no longer
  // reference any memory from the windows list, so can release the lock
  lock.unlock();
  ...
  case WM_INPUT:
    ...
    if (UINT raw_data_size = sizeof(raw_data);
        GET_RAWINPUT_CODE_WPARAM(details.wParam) !=
            RIM_INPUT || // Ignore all input sink messages (when window is not
                         // focused)
        GetRawInputData(reinterpret_cast<HRAWINPUT>(details.lParam), RID_INPUT,
                        &raw_data, &raw_data_size,
                        sizeof(raw_data.header)) == UINT(-1))
      break;
    ...
    case RIM_TYPEMOUSE:
      ...
      if (raw_input_window == s_raw_input_windows.end() ||
          (raw_input_window->second & 0x2) == 0)
        break; // Input is already handled (since legacy mouse messages are
      ...
      if (raw_data.data.mouse.usButtonFlags & RI_MOUSE_WHEEL)
        input->_mouse_wheel_delta +=
            static_cast<short>(raw_data.data.mouse.usButtonData) / WHEEL_DELTA;
      break;
    case RIM_TYPEKEYBOARD:
      ...
      // Do not block key up messages if the key down one was not blocked
      // previously
      if (input->is_blocking_keyboard_input() &&
          (raw_data.data.keyboard.Flags & RI_KEY_BREAK) != 0 &&
          raw_data.data.keyboard.VKey < 0xFF &&
          (input->_keys[raw_data.data.keyboard.VKey] & 0x04) == 0)
        is_keyboard_message = false;

      if (raw_input_window == s_raw_input_windows.end() ||
          (raw_input_window->second & 0x1) == 0)
        break; // Input is already handled by 'WM_KEYDOWN' and friends (since
               // legacy keyboard messages are enabled), so nothing to do here

      // Filter out prefix messages without a key code
      if (raw_data.data.keyboard.VKey < 0xFF)
        input->_keys[raw_data.data.keyboard.VKey] =
            (raw_data.data.keyboard.Flags & RI_KEY_BREAK) == 0 ? 0x88 : 0x08,
        input->_keys_time[raw_data.data.keyboard.VKey] = details.time;

      // No 'WM_CHAR' messages are sent if legacy keyboard messages are
      // disabled, so need to generate text input manually here Cannot use the
      // ToUnicode function always as it seems to reset dead key state and thus
      // calling it can break subsequent application input, should be fine here
      // though since the application is already explicitly using raw input
      // Since Windows 10 version 1607 this supports the 0x2 flag, which
      // prevents the keyboard state from being changed, so it is not a problem
      // there anymore either way
      if (WCHAR ch[3] = {};
          (raw_data.data.keyboard.Flags & RI_KEY_BREAK) == 0 &&
          ToUnicode(raw_data.data.keyboard.VKey,
                    raw_data.data.keyboard.MakeCode, input->_keys, ch, 2, 0x2))
  ...
  case WM_SYSKEYUP:
    assert(details.wParam > 0 && details.wParam < ARRAYSIZE(input->_keys));
    // Do not block key up messages if the key down one was not blocked
    // previously (so key does not get stuck for the application)
    if (input->is_blocking_keyboard_input() &&
        (input->_keys[details.wParam] & 0x04) == 0)
      is_keyboard_message = false;
    input->_keys[details.wParam] = 0x08;
    input->_keys_time[details.wParam] = details.time;
    break;
  case WM_LBUTTONDOWN:
  case WM_LBUTTONDBLCLK: // Double clicking generates this sequence:
                         // WM_LBUTTONDOWN -> WM_LBUTTONUP -> WM_LBUTTONDBLCLK
                         // -> WM_LBUTTONUP, so handle it like a normal down
    input->_keys[VK_LBUTTON] = 0x88;
    break;
  case WM_LBUTTONUP:
    input->_keys[VK_LBUTTON] = 0x08;
  ...
  case WM_MOUSEWHEEL:
    input->_mouse_wheel_delta +=
        GET_WHEEL_DELTA_WPARAM(details.wParam) / WHEEL_DELTA;
    break;
  case WM_XBUTTONDOWN:
    assert(HIWORD(details.wParam) == XBUTTON1 ||
           HIWORD(details.wParam) == XBUTTON2);
  ...
  return (is_mouse_message && input->is_blocking_mouse_input()) ||
...
bool reshade::input::is_key_pressed(unsigned int keycode) const {
  assert(keycode < ARRAYSIZE(_keys));
  return keycode > 0 && keycode < ARRAYSIZE(_keys) &&
         (_keys[keycode] & 0x88) == 0x88 && !is_key_repeated(keycode);
}
bool reshade::input::is_key_pressed(unsigned int keycode, bool ctrl, bool shift,
                                    bool alt, bool force_modifiers) const {
  if (keycode == 0)
    return false;
  ...
  if (force_modifiers) // Modifier state is required to match
    return key_down &&
           (ctrl == ctrl_down && shift == shift_down && alt == alt_down);
  else // Modifier state is optional and only has to match when down
    return key_down && (!ctrl || ctrl_down) && (!shift || shift_down) &&
           (!alt || alt_down);
}
bool reshade::input::is_key_released(unsigned int keycode) const {
  assert(keycode < ARRAYSIZE(_keys));
  return keycode > 0 && keycode < ARRAYSIZE(_keys) &&
         (_keys[keycode] & 0x88) == 0x08;
}
bool reshade::input::is_key_repeated(unsigned int keycode) const {
  assert(keycode < ARRAYSIZE(_keys));
  return keycode < ARRAYSIZE(_keys) && (_last_keys[keycode] & 0x80) == 0x80 &&
         (_keys[keycode] & 0x80) == 0x80;
}
...
bool reshade::input::is_mouse_button_down(unsigned int button) const {
  ...
  return is_key_down(
      VK_LBUTTON + button +
      (button < 2 ? 0 : 1)); // VK_CANCEL is being ignored by runtime
}
...
void reshade::input::next_frame() {
  ...
  // Backup key states from the last processed frame so that state transitions
  // can be identified
  ...
  // Reset any pressed down key states (apart from mouse buttons) that have not
  // been updated for more than 5 seconds Do not check mouse buttons here, since
  // 'GetAsyncKeyState' always returns the state of the physical mouse buttons,
  // not the logical ones in case they were remapped See
  ...
  for (unsigned int i = 8; i < 256; ++i)
    if ((_keys[i] & 0x80) != 0 && (time - _keys_time[i]) > 5000 &&
  ...
  if ((_keys[VK_MENU] & 0x88) != 0 && (GetKeyState(VK_MENU) & 0x8000) == 0)
    (_keys[VK_MENU] = 0x08);

  // Update print screen state (there is no key down message, but the key up one
  // is received via the message queue)
  if ((_keys[VK_SNAPSHOT] & 0x80) == 0 &&
      (GetAsyncKeyState(VK_SNAPSHOT) & 0x8000) != 0)
    (_keys[VK_SNAPSHOT] = 0x88), (_keys_time[VK_SNAPSHOT] = time);

  // Run through all forms of input blocking for all windows and establish
  // whether any of them are blocking input
  const std::shared_lock<std::shared_mutex> lock(s_windows_mutex);

  const auto mouse_predicate =
      [&](const std::pair<HWND, std::weak_ptr<reshade::input>> &input_window) {
        return !input_window.second.expired() &&
               input_window.second.lock()->is_blocking_mouse_input();
      };
  const auto keyboard_predicate =
      [&](const std::pair<HWND, std::weak_ptr<reshade::input>> &input_window) {
        return !input_window.second.expired() &&
               input_window.second.lock()->is_blocking_keyboard_input();
      };
  const auto cursor_predicate =
      [&](const std::pair<HWND, std::weak_ptr<reshade::input>> &input_window) {
        return !input_window.second.expired() &&
               input_window.second.lock()->is_immobilizing_cursor();
      };

  s_blocking_global_mouse_input.store(
      std::any_of(s_windows.cbegin(), s_windows.cend(), mouse_predicate));
  s_blocking_global_keyboard_input.store(
      std::any_of(s_windows.cbegin(), s_windows.cend(), keyboard_predicate));
  s_immobilizing_global_cursor_move.store(
      std::any_of(s_windows.cbegin(), s_windows.cend(), cursor_predicate));
}
...
std::string reshade::input::key_name(unsigned int keycode) {
  ...
  if (keycode == VK_HOME && LOBYTE(GetKeyboardLayout(0)) == LANG_GERMAN)
    return "Pos1";
...
std::string reshade::input::key_name(const unsigned int key[4]) {
  ...
  return (key[1] ? "Ctrl + " : std::string()) +
         (key[2] ? "Shift + " : std::string()) +
         (key[3] ? "Alt + " : std::string()) + key_name(key[0]);
}
...
void reshade::input::block_mouse_input(bool enable) {
  // Some games setup ClipCursor with a tiny area which could make the cursor
  // stay in that area instead of the whole window
  if (enable) {
    // This will call into 'HookClipCursor' below, so back up and restore
    // rectangle
    const RECT last_clip_cursor = s_last_clip_cursor;
    ClipCursor(nullptr);
    s_last_clip_cursor = last_clip_cursor;
  } else if ((s_last_clip_cursor.right - s_last_clip_cursor.left) != 0 &&
...
bool is_immobilizing_cursor(
    reshade::input::window_handle target = reshade::input::any_window) {
  if (target != reshade::input::any_window) {
    const std::shared_lock<std::shared_mutex> lock(s_windows_mutex);

    const auto predicate =
        [&](const std::pair<HWND, std::weak_ptr<reshade::input>>
                &input_window) {
          return !input_window.second.expired() &&
                 input_window.first == target &&
                 input_window.second.lock()->is_immobilizing_cursor();
        };
...
extern "C" BOOL WINAPI HookGetMessageA(LPMSG lpMsg, HWND hWnd,
  ...
  // Implement 'GetMessage' with an additional trigger event (see also
  // DLL_PROCESS_DETACH in dllmain.cpp for more explanation)
  while (!PeekMessageA(lpMsg, hWnd, wMsgFilterMin, wMsgFilterMax, PM_REMOVE)) {
    assert(g_exit_event != nullptr);

    if (MsgWaitForMultipleObjects(1, &g_exit_event, FALSE, INFINITE, mask) !=
        (WAIT_OBJECT_0 + 1)) {
      std::memset(lpMsg, 0, sizeof(MSG)); // Clear message structure, so
  ...
  if (result <
      0) // If there is an error, the return value is negative
         // (https://docs.microsoft.com/windows/win32/api/winuser/nf-winuser-getmessage)
    return result;
...
extern "C" BOOL WINAPI HookGetMessageW(LPMSG lpMsg, HWND hWnd,
                                       UINT wMsgFilterMin, UINT wMsgFilterMax) {
#if 1
  DWORD mask = QS_ALLINPUT;
  if (wMsgFilterMin != 0 || wMsgFilterMax != 0) {
...
extern "C" BOOL WINAPI HookPeekMessageA(LPMSG lpMsg, HWND hWnd,
                                        UINT wMsgFilterMin, UINT wMsgFilterMax,
                                        UINT wRemoveMsg) {
...
extern "C" BOOL WINAPI HookPeekMessageW(LPMSG lpMsg, HWND hWnd,
                                        UINT wMsgFilterMin, UINT wMsgFilterMax,
                                        UINT wRemoveMsg) {
...
extern "C" BOOL WINAPI HookPostMessageA(HWND hWnd, UINT Msg, WPARAM wParam,
                                        LPARAM lParam) {
...
extern "C" BOOL WINAPI HookPostMessageW(HWND hWnd, UINT Msg, WPARAM wParam,
                                        LPARAM lParam) {
  if (is_blocking_mouse_input(hWnd) && Msg == WM_MOUSEMOVE)
    return TRUE;
...
extern "C" BOOL WINAPI HookRegisterRawInputDevices(
    PCRAWINPUTDEVICE pRawInputDevices, UINT uiNumDevices, UINT cbSize) {
#if RESHADE_VERBOSE_LOG
  reshade::log::message(reshade::log::level::debug,
                        "Redirecting RegisterRawInputDevices(pRawInputDevices "
                        "= %p, uiNumDevices = %u, cbSize = %u) ...",
  ...
  for (UINT i = 0; i < uiNumDevices; ++i) {
...
#if RESHADE_VERBOSE_LOG
    reshade::log::message(reshade::log::level::debug,
                          "> Dumping device registration at index %u:", i);
    reshade::log::message(reshade::log::level::debug,
                          "  "
                          "+-----------------------------------------+---------"
                          "--------------------------------+");
    ...
    reshade::input::register_window_with_raw_input(
        device.hwndTarget,
        device.usUsage == 0x06 && (device.dwFlags & RIDEV_NOLEGACY) != 0,
        device.usUsage == 0x02 && (device.dwFlags & RIDEV_NOLEGACY) != 0);
  }

  static const auto trampoline =
      reshade::hooks::call(HookRegisterRawInputDevices);
  if (!trampoline(pRawInputDevices, uiNumDevices, cbSize)) {
    reshade::log::message(reshade::log::level::warning,
                          "RegisterRawInputDevices failed with error code %lu.",
                          GetLastError());
    return FALSE;
  }
...
extern "C" BOOL WINAPI HookClipCursor(const RECT *lpRect) {
  ...
  // Some applications clip the mouse cursor, so disable that while we want full
  // control over mouse input
...
extern "C" BOOL WINAPI HookGetCursorPosition(LPPOINT lpPoint) {
  // Allow the game to see the real cursor position if it's not busy using the
  // wrong API for mouselook... (i.e. no calls to SetCursorPos in a certain
  // period of time)
  const auto now = std::chrono::high_resolution_clock::now();
  const bool recently_warped =
      (is_immobilizing_cursor() &&
       std::chrono::duration_cast<std::chrono::milliseconds>(now -
                                                             s_last_cursor_warp)
               .count() < reshade::input::input_grace_period_ms);
  if (is_blocking_mouse_input() || recently_warped) {
    assert(lpPoint != nullptr);

    // Return the last cursor position before we started immobilizing the mouse
    // cursor
...
// This variety of RawInput does not use WM_INPUT, and a hook is necessary to
// block these inputs
extern "C" UINT WINAPI HookGetRawInputBuffer(PRAWINPUT pData, PUINT pcbSize,
                                             UINT cbSizeHeader) {
  ...
  if (pData != nullptr) {
    ...
    const int max_items =
        ((static_cast<size_t>(*pcbSize) * 16) / sizeof(RAWINPUT));
    int count = 0;
    ...
    *pcbSize = 0;
    ...
    // Common usage involves calling this with a wrong sized buffer, then
    // calling it again...
    ...
    for (int i = 0; i < temp_ret; i++) {
      ...
      if (remove == false) {
        ...
        // Clearing all bytes above would have set the type to mouse, and some
        // games
        //   will actually read data coming from RawInput even when the size
        //   returned is 0!
        pItem->header.dwType = keyboard ? RIM_TYPEKEYBOARD
                               : mouse  ? RIM_TYPEMOUSE
                                        : RIM_TYPEHID;

        // Supplying an invalid device will early-out SDL before it calls HID
        // APIs to try
    ...
    *pcbSize = static_cast<UINT>(reinterpret_cast<uintptr_t>(pOutput) -
...
static LRESULT CALLBACK proxy_mouse_proc(int nCode, WPARAM wParam,
                                         LPARAM lParam) {
  ...
  if (nCode == HC_ACTION || nCode == HC_NOREMOVE) {
    ...
    if (is_blocking_mouse_input()) {
      ...
      HOOKPROC hook_fn =
          reshade::hooks::win32::real_mouse_procs.count(thread_id) &&
                  reshade::hooks::win32::real_mouse_procs.at(thread_id) !=
                      nullptr
              ? reshade::hooks::win32::real_mouse_procs.at(thread_id)
              : reshade::hooks::win32::real_mouse_proc;
...
static LRESULT CALLBACK proxy_low_level_mouse_proc(int nCode, WPARAM wParam,
                                                   LPARAM lParam) {
  // The only documented codes
...
static LRESULT CALLBACK proxy_keyboard_proc(int nCode, WPARAM wParam,
                                            LPARAM lParam) {
  // The only documented codes
  assert(nCode < 0 || nCode == HC_ACTION || nCode == HC_NOREMOVE);
  ...
  if (nCode == HC_ACTION || nCode == HC_NOREMOVE) {
    ...
    if (is_blocking_keyboard_input()) {
      ...
      DWORD thread_id = GetCurrentThreadId();
      HOOKPROC hook_fn =
          (reshade::hooks::win32::real_keyboard_procs.count(thread_id) &&
           reshade::hooks::win32::real_keyboard_procs.at(thread_id) != nullptr)
              ? reshade::hooks::win32::real_keyboard_procs.at(thread_id)
              : reshade::hooks::win32::real_keyboard_proc;
...
static LRESULT CALLBACK proxy_low_level_keyboard_proc(int nCode, WPARAM wParam,
                                                      LPARAM lParam) {
  // The only documented codes
  assert(nCode < 0 || nCode == HC_ACTION);
...
extern "C" HHOOK WINAPI HookSetWindowsHookExW(int idHook, HOOKPROC lpfn,
                                              HINSTANCE hmod,
                                              DWORD dwThreadId) {
  ...
  reshade::log::message(reshade::log::level::info,
                        "Redirecting SetWindowsHookExW(idHook = %d, lpfn = %p, "
                        "hmod = %p, dwThreadId = %d) ...",
  ...
  case WH_KEYBOARD_LL: {
    if (idHook == WH_KEYBOARD || idHook == WH_KEYBOARD_LL) {
      ...
      if (dwThreadId != 0) {
        if (!reshade::hooks::win32::real_keyboard_procs.count(dwThreadId) ||
            reshade::hooks::win32::real_keyboard_procs[dwThreadId] == nullptr) {
          ...
          reshade::log::message(reshade::log::level::warning,
                                "Keyboard proc already exists for thread %d",
                                dwThreadId);
      }
        ...
        reshade::log::message(reshade::log::level::warning,
                              "Global keyboard proc already exists");

      if (install)
        lpfn = (idHook == WH_KEYBOARD ? proxy_keyboard_proc
                                      : proxy_low_level_keyboard_proc);
    }
  } break;
  ...
  case WH_MOUSE_LL: {
    if (idHook == WH_MOUSE || idHook == WH_MOUSE_LL) {
      ...
      if (dwThreadId != 0) {
        if (!reshade::hooks::win32::real_mouse_procs.count(dwThreadId) ||
            reshade::hooks::win32::real_mouse_procs[dwThreadId] == nullptr) {
          ...
          reshade::log::message(reshade::log::level::warning,
                                "Mouse proc already exists for thread %d",
                                dwThreadId);
      }
        ...
        reshade::log::message(reshade::log::level::warning,
                              "Global mouse proc already exists");

      if (install)
        lpfn = (idHook == WH_MOUSE ? proxy_mouse_proc
                                   : proxy_low_level_mouse_proc);
    }
  } break;
  }
...
extern "C" HHOOK WINAPI HookSetWindowsHookExA(int idHook, HOOKPROC lpfn,
                                              HINSTANCE hmod,
                                              DWORD dwThreadId) {
  ...
  reshade::log::message(reshade::log::level::info,
                        "Redirecting SetWindowsHookExA(idHook = %d, lpfn = %p, "
                        "hmod = %p, dwThreadId = %d) ...",
                        idHook, lpfn, hmod, dwThreadId);
...
extern "C" BOOL WINAPI HookUnhookWindowsHookEx(HHOOK hhk) {
  ...
  reshade::log::message(reshade::log::level::info,
                        "Redirecting UnhookWindowsHookEx(hhk = %p) ...", hhk);

  for (auto &hook : reshade::hooks::win32::real_mouse_hooks) {
    if (hook.second == hhk) {
      reshade::log::message(reshade::log::level::info,
                            "Proxy mouse hook removed for thread %d",
                            hook.first);
  ...
  for (auto &hook : reshade::hooks::win32::real_keyboard_hooks) {
    if (hook.second == hhk) {
      reshade::log::message(reshade::log::level::info,
                            "Proxy keyboard hook removed for thread %d",
                            hook.first);
